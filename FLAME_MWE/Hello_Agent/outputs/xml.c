				...
				int * itno)
{
	...
	int FLAME_in_name = 0;
	int in_Sender_agent = 0;
	int in_Receiver_agent = 0;
	
	int in_id = 0;
	
	xmachine_memory_Sender * current_Sender_agent = NULL;
	xmachine_memory_Receiver * current_Receiver_agent = NULL;
	...
	while(c != (char)EOF)
	{
		...
		if(c == '>')
		{
			...
			{	
				...
				{
					if(current_Sender_agent == NULL) { printf("Memory error reading Sender agent\n"); exit(0); }
					...
					if( flag == 0 )
					{
						...
						add_Sender_agent_internal(current_Sender_agent, Sender_start_Sender_state);
				...
				else if(strcmp(agentname, "Receiver") == 0)
				{
					if(current_Receiver_agent == NULL) { printf("Memory error reading Receiver agent\n"); exit(0); }
					...
					{
						/* Next line should be commented out? */
						add_Receiver_agent_internal(current_Receiver_agent, Receiver_start_Receiver_state);
		...
		else if(c == '<')
		{
			...
			if(FLAME_in_xagent == 1)
			{
				if(FLAME_in_name == 1)
				{
					...
					if(strcmp(agentname, "Sender") == 0)
					{
						current_Sender_agent = init_Sender_agent();
						in_Sender_agent = 1;
					}
					else if(strcmp(agentname, "Receiver") == 0)
					{
						current_Receiver_agent = init_Receiver_agent();
						in_Receiver_agent = 1;
					}
					else
					{
				...
				else if(in_Sender_agent == 1)
				{
					if(in_id) { current_Sender_agent->id = atoi(buffer); }
				 }else if(in_Receiver_agent == 1)
				{
					if(in_id) { current_Receiver_agent->id = atoi(buffer); }
				 }
			}
			index = 0;
			buffer[index] = '\0';
					   ...
					   int partition_method, int flag)
{
    ...
    *itno = 0;
		...
		else if(c == '<')
		{
			...
			if(FLAME_in_outputs == 1)
			{
				if(FLAME_in_output == 1)
				{
					...
					if(FLAME_in_name == 1)
					{
						if(strcmp("Sender", buffer) == 0) current_FLAME_output->type = 1;
						else if(strcmp("Receiver", buffer) == 0) current_FLAME_output->type = 2;
						else 
						{
	...
	for(current_FLAME_output = FLAME_outputs; current_FLAME_output != NULL; current_FLAME_output = current_FLAME_output->next)
	{
		...
		else if(current_FLAME_output->type == 1) printf("agent' name='Sender");
		else if(current_FLAME_output->type == 2) printf("agent' name='Receiver");
...
void write_Sender_agent(FILE *file, xmachine_memory_Sender * current)
{
	char data[1000];
	fputs("<xagent>\n" , file);
	fputs("<name>Sender</name>\n", file);
		fputs("<id>", file);
...
void write_Receiver_agent(FILE *file, xmachine_memory_Receiver * current)
{
	char data[1000];
	fputs("<xagent>\n" , file);
	fputs("<name>Receiver</name>\n", file);
		fputs("<id>", file);
	sprintf(data, "%i", current->id);
	fputs(data, file);
	fputs("</id>\n", file);
...
void FLAME_write_xml(char * location, int iteration_number, int * output_types, int output_type_size)
{
	...
	if(FLAME_integer_in_array(0, output_types, output_type_size) || FLAME_integer_in_array(1, output_types, output_type_size))
	{
		current_xmachine_Sender_holder = Sender_start_Sender_state->agents;
			...
			{
				write_Sender_agent(file, current_xmachine_Sender_holder->agent);

				current_xmachine_Sender_holder = current_xmachine_Sender_holder->next;
			}
	}
	...
	if(FLAME_integer_in_array(0, output_types, output_type_size) || FLAME_integer_in_array(2, output_types, output_type_size))
	{
		current_xmachine_Receiver_holder = Receiver_start_Receiver_state->agents;
			...
			{
				write_Receiver_agent(file, current_xmachine_Receiver_holder->agent);

				current_xmachine_Receiver_holder = current_xmachine_Receiver_holder->next;
			}
	}

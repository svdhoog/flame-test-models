...
void unittest_sender_sends_mesg_start_Sender_end_Sender()
{
	
	
	//return sender_sends_mesg();
}

void unittest_receiver_reads_mesg_start_Receiver_end_Receiver()
{
	int rc;
...
int rc;
	...
	FLAME_m_hello_composite_params = NULL;

	    rc = MB_Create(&b_hello, sizeof(m_hello));
	    #ifdef ERRCHECK
	    if (rc != MB_SUCCESS)
	    {
	       fprintf(stderr, "ERROR: Could not create 'hello' board\n");
	       switch(rc) {
	           case MB_ERR_INVALID:
	...
	Sender_end_Sender_state = init_Sender_state();

	Sender_start_Sender_state = init_Sender_state();

	Receiver_end_Receiver_state = init_Receiver_state();

	Receiver_start_Receiver_state = init_Receiver_state();
...
{
	xmachine_memory_Sender_state * current = (xmachine_memory_Sender_state *)malloc(sizeof(xmachine_memory_Sender_state));
	CHECK_POINTER(current);
...
{
	xmachine_memory_Sender * current = (xmachine_memory_Sender *)malloc(sizeof(xmachine_memory_Sender));
	CHECK_POINTER(current);
...
{
	current_xmachine_Sender = (xmachine_memory_Sender *)malloc(sizeof(xmachine_memory_Sender));
	CHECK_POINTER(current);

		current_xmachine_Sender->id = 0;
...
void unittest_free_Sender_agent()
{
	...
	free(current_xmachine_Sender);
}

void free_Sender_agents()
{
	...
	while(current_xmachine_Sender_holder)
	{
		...
		free_Sender_agent(current_xmachine_Sender_holder, Sender_end_Sender_state);
...
void free_Sender_states()
{
	free(Sender_end_Sender_state);
	free(Sender_start_Sender_state);
}
...
void transition_Sender_agent(xmachine_memory_Sender_holder * tmp, xmachine_memory_Sender_state * from_state, xmachine_memory_Sender_state * to_state)
{
	...
	add_Sender_agent_internal(tmp->agent, to_state);
	free(tmp);
}

void add_Sender_agent_internal(xmachine_memory_Sender * agent, xmachine_memory_Sender_state * state)
{
	xmachine_memory_Sender_holder * current = (xmachine_memory_Sender_holder *)malloc(sizeof(xmachine_memory_Sender_holder));
	CHECK_POINTER(current);
...
/** \fn void add_Sender_agent(int id)
 * \brief Add Sender X-machine to the current being used X-machine list.
 * \param id Variable for the X-machine memory.
 */
void add_Sender_agent(int id)
{
	xmachine_memory_Sender * current;

	current = init_Sender_agent();
	/* new line added to handle dynamic agent creation*/
	current_xmachine_Sender_next_state = Sender_start_Sender_state;
	add_Sender_agent_internal(current, current_xmachine_Sender_next_state);
...
{
	xmachine_memory_Receiver_state * current = (xmachine_memory_Receiver_state *)malloc(sizeof(xmachine_memory_Receiver_state));
	CHECK_POINTER(current);
...
{
	xmachine_memory_Receiver * current = (xmachine_memory_Receiver *)malloc(sizeof(xmachine_memory_Receiver));
	CHECK_POINTER(current);
...
{
	current_xmachine_Receiver = (xmachine_memory_Receiver *)malloc(sizeof(xmachine_memory_Receiver));
	CHECK_POINTER(current);

		current_xmachine_Receiver->id = 0;
...
void unittest_free_Receiver_agent()
{
	...
	free(current_xmachine_Receiver);
}

void free_Receiver_agents()
{
	...
	while(current_xmachine_Receiver_holder)
	{
		...
		free_Receiver_agent(current_xmachine_Receiver_holder, Receiver_end_Receiver_state);
...
void free_Receiver_states()
{
	free(Receiver_end_Receiver_state);
	free(Receiver_start_Receiver_state);
}
...
void transition_Receiver_agent(xmachine_memory_Receiver_holder * tmp, xmachine_memory_Receiver_state * from_state, xmachine_memory_Receiver_state * to_state)
{
	...
	add_Receiver_agent_internal(tmp->agent, to_state);
	free(tmp);
}

void add_Receiver_agent_internal(xmachine_memory_Receiver * agent, xmachine_memory_Receiver_state * state)
{
	xmachine_memory_Receiver_holder * current = (xmachine_memory_Receiver_holder *)malloc(sizeof(xmachine_memory_Receiver_holder));
	CHECK_POINTER(current);
...
/** \fn void add_Receiver_agent(int id)
 * \brief Add Receiver X-machine to the current being used X-machine list.
 * \param id Variable for the X-machine memory.
 */
void add_Receiver_agent(int id)
{
	xmachine_memory_Receiver * current;

	current = init_Receiver_agent();
	/* new line added to handle dynamic agent creation*/
	current_xmachine_Receiver_next_state = Receiver_start_Receiver_state;
	add_Receiver_agent_internal(current, current_xmachine_Receiver_next_state);
...
void set_id(int id)
{
	if(current_xmachine->xmachine_Sender) (*current_xmachine->xmachine_Sender).id = id;
	if(current_xmachine->xmachine_Receiver) (*current_xmachine->xmachine_Receiver).id = id;
}
...
int get_id()
{
	if(current_xmachine->xmachine_Sender) return (*current_xmachine->xmachine_Sender).id;
	if(current_xmachine->xmachine_Receiver) return (*current_xmachine->xmachine_Receiver).id;
...
{
    double value = 0.0;
    /*if (current_xmachine->xmachine_Sender) value = current_xmachine->xmachine_Sender->;*/
    /*if (current_xmachine->xmachine_Receiver) value = current_xmachine->xmachine_Receiver->;*/
...
{
    int value = 0;
    /*if (current_xmachine->xmachine_Sender) value = current_xmachine->xmachine_Sender->id;*/
    /*if (current_xmachine->xmachine_Receiver) value = current_xmachine->xmachine_Receiver->id;*/
...
{
    double value = 0.0;
    /*if (current_xmachine->xmachine_Sender) value = current_xmachine->xmachine_Sender->0.0;*/
    /*if (current_xmachine->xmachine_Receiver) value = current_xmachine->xmachine_Receiver->0.0;*/
...
void clean_up(int code)
{
	...
	rc = MB_Delete(&b_hello);
	#ifdef ERRCHECK
    if (rc != MB_SUCCESS)
    {
       fprintf(stderr, "ERROR: Could not delete 'hello' board\n");
       switch(rc) {
           case MB_ERR_INVALID:
